...
pub fn ban_button(user_id: UserId, reason: Option<&str>) -> Component {
...
pub fn kick_button(user_id: UserId, reason: Option<&str>) -> Component {
    let mut action = Action::new();
    action.set_user_id(user_id.get());
    if let Some(reason) = reason {
        action.set_reason(reason.to_owned());
...
pub fn create_action_button(
    ...
    proto.mut_action_button().set_required_permissions(permissions.bits());
    proto.mut_action_button().mut_actions().action.extend(actions);
