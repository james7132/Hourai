    ...
    loop {
        ...
        for message in messages {
            oldest = oldest
                ...
                .or(Some(message.id));
            if message.timestamp.as_secs() < limit as i64 || tx.unbounded_send(message).is_err() {
                return Ok(());
            }
        }
    }
...
pub(super) async fn role_allow(ctx: &CommandContext, storage: &Storage) -> Result<Response> {
    ctx.defer().await?;
    let guild_id = ctx.guild_id()?;
    if !ctx.has_user_permission(Permissions::MANAGE_ROLES) {
        anyhow::bail!(InteractionError::MissingPermission("Manage Roles"));
    ...
    let mut config: hourai::proto::guild_configs::RoleConfig =
        storage.redis().guild(guild_id).configs().get().await?;
    let mut ids = config.get_self_serve_role_ids().to_vec();
    if !ids.contains(&role_id.get()) {
        ids.push(role_id.get());
        config.set_self_serve_role_ids(ids);
        storage
            .redis()
            .guild(guild_id)
            .configs()
    ...
    Ok(Response::direct().content(format!("Allowed self-serve for <@&{}>.", role_id)))
}

pub(super) async fn role_forbid(ctx: &CommandContext, storage: &Storage) -> Result<Response> {
    ctx.defer().await?;
    let guild_id = ctx.guild_id()?;
    if !ctx.has_user_permission(Permissions::MANAGE_ROLES) {
        anyhow::bail!(InteractionError::MissingPermission("Manage Roles"));
    ...
    let ids: Vec<u64> = config
        .get_self_serve_role_ids()
        .iter()
        .cloned()
        .filter(|id| *id != role_id.get())
        .collect();
    config.set_self_serve_role_ids(ids);
    storage
        .redis()
        ...
        .set(config)
        .await?;
    Ok(Response::direct().content(format!("Disallowed self-serve for <@&{}>.", role_id)))
}

pub(super) async fn role_get(ctx: &CommandContext, storage: &Storage) -> Result<Response> {
    ctx.defer().await?;
    let guild_id = ctx.guild_id()?;
    let role_id = ctx.get_role("role")?;
    let config: hourai::proto::guild_configs::RoleConfig =
        storage.redis().guild(guild_id).configs().get().await?;
    if !config.get_self_serve_role_ids().contains(&role_id.get()) {
        return Ok(Response::ephemeral()
            .content(format!("<@&{}> is not configured for self-serve.", role_id)));
    }
...
pub(super) async fn role_drop(ctx: &CommandContext, storage: &Storage) -> Result<Response> {
    ctx.defer().await?;
    let guild_id = ctx.guild_id()?;
    let role_id = ctx.get_role("role")?;
    let config: hourai::proto::guild_configs::RoleConfig =
