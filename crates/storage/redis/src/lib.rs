...
impl MessageCache {
    ...
    pub async fn bulk_delete(
        ...
        let keys: Vec<CacheKey> = ids
            ...
            .collect();
        let _: () = self.0.connection_mut().del(keys).await?;
        Ok(())
    }
}
...
impl VoiceStateCache {
    pub async fn update_guild(&mut self, guild: &Guild) -> Result<()> {
        ...
        let _: () = pipe.query_async(self.0.redis.connection_mut()).await?;
        Ok(())
    }
...
impl GuildCache {
    ...
    pub async fn save(&mut self, guild: &hourai::models::guild::Guild) -> Result<()> {
        ...
        let _: () = pipe.query_async(self.redis.connection_mut()).await?;
        Ok(())
    }
