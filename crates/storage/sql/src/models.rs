...
impl Tag {
    pub fn get<'a>(guild_id: Id<GuildMarker>, tag: &str) -> SqlQueryAs<'a, Self> {
        sqlx::query_as("SELECT * FROM tags WHERE guild_id = $1 AND tag = $2")
            .bind(guild_id.get() as i64)
            .bind(tag.to_lowercase())
    }

    pub fn list<'a>(guild_id: Id<GuildMarker>) -> SqlQueryAs<'a, (String,)> {
        sqlx::query_as("SELECT tag FROM tags WHERE guild_id = $1 ORDER BY tag")
            .bind(guild_id.get() as i64)
    }

    pub fn set<'a>(guild_id: Id<GuildMarker>, tag: &str, response: &'a str) -> SqlQuery<'a> {
        sqlx::query(
            "INSERT INTO tags (guild_id, tag, response) VALUES ($1, $2, $3) \
             ON CONFLICT (guild_id, tag) DO UPDATE SET response = excluded.response",
        )
        .bind(guild_id.get() as i64)
        .bind(tag.to_lowercase())
        .bind(response)
    ...
    pub fn delete<'a>(guild_id: Id<GuildMarker>, tag: &str) -> SqlQuery<'a> {
        sqlx::query("DELETE FROM tags WHERE guild_id = $1 AND tag = $2")
            .bind(guild_id.get() as i64)
            .bind(tag.to_lowercase())
    }
}
...
pub struct AdminConfig {
    pub id: i64,
    pub source_bans: bool,
    pub is_blocked: bool,
}

impl AdminConfig {
    pub fn fetch<'a>(id: i64) -> SqlQueryAs<'a, Self> {
        sqlx::query_as("SELECT * FROM admin_configs WHERE id = $1").bind(id)
    }
}
...
impl Alias {
    pub fn get<'a>(guild_id: Id<GuildMarker>, name: &str) -> SqlQueryAs<'a, Self> {
        sqlx::query_as("SELECT * FROM aliases WHERE guild_id = $1 AND name = $2")
            .bind(guild_id.get() as i64)
            .bind(name.to_lowercase())
    }

    pub fn list<'a>(guild_id: Id<GuildMarker>) -> SqlQueryAs<'a, (String,)> {
        sqlx::query_as("SELECT name FROM aliases WHERE guild_id = $1 ORDER BY name")
            .bind(guild_id.get() as i64)
    }

    pub fn set<'a>(guild_id: Id<GuildMarker>, name: &str, content: &'a str) -> SqlQuery<'a> {
        sqlx::query(
            "INSERT INTO aliases (guild_id, name, content) VALUES ($1, $2, $3) \
    ...
    pub fn delete<'a>(guild_id: Id<GuildMarker>, name: &str) -> SqlQuery<'a> {
        sqlx::query("DELETE FROM aliases WHERE guild_id = $1 AND name = $2")
            .bind(guild_id.get() as i64)
            .bind(name.to_lowercase())
    }
}
